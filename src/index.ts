...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    ...
    settings?.installSection?.(ctx, PROMPT_INJECT_NAMESPACE, PromptInjectConfig, {}, {
      ...
      onChange: () => { /* 每步 assemble/每次注入都实时读取 */ },
    })
  })

  // 2) system 通道:所有走 dsh system 的 agent(含 codebuddy 子代理)。
  ctx.inject(['systemPrompt'], (systemCtx) => {
    const systemPrompt = systemCtx.get('systemPrompt') as {
      section?: (options: { name: string; order: number; text: (context: unknown) => string }) => () => void
    } | undefined
  ...
  ctx.on('agent/pre-step', async (payload, next) => {
    ...
    if (payload.agent.options.provider !== 'agy') return downstream
